...
export default {
    ...
    mounted() {
        ...
        function StartCall(parterName, createOffer) {
            ...
            if (localStream) {
                localStream.getTracks().forEach((track) => {
                    pc[parterName].addTrack(track, localStream); //shouldtriggernegotiationneededevent
                });
                console.log(pc);
            } else {
                //否则需要重新启动摄像头并获取
            ...
            //当需要你通过信令服务器将一个ICE候选发送给另一个对等端时，本地ICE层将会调用你的icecandidate事件处理程序。有关更多信息，请参阅SendingICEcandidates以查看此示例的代码。
            ...
            //当向连接中添加磁道时，track事件的此处理程序由本地WebRTC层调用。例如，可以将传入媒体连接到元素以显示它。详见Receivingnewstreams。
        ...
        var socket = this.$io_video;
        //var socket = io("http://localhost:9998")
        socket.on("connect", () => {
            InitCamera();
            //输出内容其中socket.id是当前socket连接的唯一ID
            console.log("connect" + socket.id);
            $("#user-id").text("我的id:"+socket.id);
            pc.push(socket.id);
            socket.emit("newusergreet", {
                sender: socket.id,
                msg: "helloworld",
            });
            socket.on("needconnect", (data) => {
                console.log(data);
                //创建新的li并添加到用户列表中
                let li = $("<li></li>")
                    .text(data.sender)
                ...
                button.appendTo(li); //监听按钮的点击事件,这是个demo需要添加很多东西，比如不能重复拨打已经连接的用户
            ...
            socket.on("userdisconnected", (socket_id) => {
                ...
                $('#user-listli[user-id="' + socket_id + '"]').remove();
            });
            //链接吧..
            ...
            //如果是icecandidates的协商信息
            socket.on("icecandidates", (data) => {
                console.log("icecandidate:" + data.candidate);
                //{candidate:candidate,to:partnerName,sender:socketID}
                //如果icecandidate非空（当candidate为空时，那么本次协商流程到此结束了）
                if (data.candidate) {
                    var candidate = new RTCIceCandidate(data.candidate);
                    //讲对方发来的协商信息保存
                    pc[data.sender].addIceCandidate(candidate).catch(); //catcherrfunctionempty
                }
            });
        });
    },
